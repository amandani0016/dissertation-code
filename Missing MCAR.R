...
LASSO_model_fitting <- function(data){
  # adaptive lasso 
...
# data generation function for MCAR
simulate_lasso_data <- function(condition, repli, N, b0, b1, beta1, beta2, beta3, cor5_7){
  num_of_pred = 10
  ...
  for (re in 1: repli){
    ...
    y <- -1 + beta1 * x1 + beta2 * x2 + beta3 * x3 + rlogis(N, mean = 0, sd = 1)
    ...
    # Create Missing Data Patterns for Other Predictors: x2, x3 & d1 - d7 
    for(k in 1:9){
      u <- runif(N,0,1)
    ...
    # Impose missing values on all other predictors x2, x3 & d1-d7
    for(k in 1:9){
    ...
    if (sum(pooling_para_result_selection) == 0){
      ...
      formu = paste("y ~ ", equ)
      print(formu)
      relax_lasso_model <- glm(formu, data = all_imp_data, family = binomial(link = "probit"))
    ...
    if(sum(mi_al_selection_result) == 0){
      ...
      print(formu_ave)
      relax_lasso_model <- glm(formu_ave, data = all_imp_data, family = binomial(link = "probit"))
      result = as.matrix(coef(relax_lasso_model))
      rn = rownames(result)
  ...
  write.table(output_pre_replication,file=paste('/Users/yibinni/Desktop/LASSO All the way/MissingSimulation/output/',condition,'_d', re, 'result_output_pre_rep', '.csv',sep=''),row.names = FALSE,col.names = FALSE,sep = "\t")
  ...
  write.table(all_output,file=paste('/Users/yibinni/Desktop/LASSO All the way/MissingSimulation/output/',condition,'_d', re, 'result_output', '.csv',sep=''),row.names = TRUE,col.names = TRUE,sep = "\t")
}
...
for (con in 1:100){
  ...
  simulate_lasso_data(conditions$cond[con], conditions$replication_num[con], conditions$sample_sizes[con], conditions$b0[con], 
                      conditions$b1[con], conditions$beta1[con], conditions$beta2[con], conditions$beta3[con], conditions$cor5_7[con])
}
