...
# data generation function for MAR with d5
simulate_lasso_data <- function(condition, repli, N, b0, b1, beta1, beta2, beta3, cor5_7, d5_mis_pre){
  num_of_pred = 10
  ...
  for (re in 1: repli){
    ...
    u_x1 <- runif(N,0,1)
    p_x1 <- exp(b0 + b1*d5)/(1 + exp(b0 + b1*d5))
    ...
    # create missing data for d5
    u_d5 = c()
    miss_d5 <- c()
    u_d5 = runif(N, 0, 1)
    
    for(i in 1:N){
      if(u_d5[i] < d5_mis_pre){miss_d5[i] <- 0}
      else(miss_d5[i] <- 1)
    }
    ...
    # Impose missing values on target predictor: d5
    for(i in 1:N){if(miss_d5[i] == 0){d5_miss[i] <- NA}}
    ...
    for(k in 1:8){
      ...
      if(k == 3){
        for(i in 1:N){if(single_pred_miss[i] == 0){d1_miss[i] <- NA}}}
    ...
    data <- data.frame(miss_x1, d5, p_x1)
    model = glm(miss_x1~d5, data = data)
    ...
    if (sum(pooling_para_result_selection) == 0){
      ...
      formu = paste("y ~ ", equ)
      print(formu)
      relax_lasso_model <- glm(formu, data = all_imp_data, family = binomial(link = "logit"))
  ...
  write.table(output_pre_replication,file=paste('/Users/yibinni/Desktop/LASSO All the way/MissingSimulation/output3/',condition,'_d', re, 'result_output_pre_rep', '.csv',sep=''),row.names = TRUE,col.names = FALSE,sep = "\t")
  ...
  write.table(all_output,file=paste('/Users/yibinni/Desktop/LASSO All the way/MissingSimulation/output3/',condition,'_d', re, 'result_output', '.csv',sep=''),row.names = FALSE,col.names = TRUE,sep = "\t")
...
for (con in 301:500){
  set.seed(con)
  simulate_lasso_data(conditions$cond[con], 10, conditions$sample_sizes[con], conditions$b0[con], 
                      conditions$b1[con], conditions$beta1[con], conditions$beta2[con], conditions$beta3[con], conditions$cor5_7[con], conditions$d5_mis_pre[con])
}
